...
// Text styling
d3.select("h1").style("color", "black").style("font-family", "lato");

d3.select("h3").style("color", "black").style("font-family", "lato");
...
//Read the data
d3.csv(
  "https://raw.githubusercontent.com/UlyssesLin/world_bank/main/data/regressionData/Forecast_newer.csv",
  function (d) {
    return {
      country: d.country,
      year: d3.timeParse(timeFormatter)(d.year),
      actualEstimate: d.actualEstimate,
      upperInterval: d.upperInterval,
      lowerInterval: d.lowerInterval,
    };
  }
).then(function (data) {
  ...
  var timeLine = d3.extent(data, function (d) {
    return d.year;
  });
  ...
  function update(selected_country) {
    ...
    var startText = d3.timeParse(timeFormatter)("1990");
    ...
    svgLine
      ...
      .datum(
        data.filter(function (d) {
          return d.country == selected_country && d.year >= forcastStart;
        })
      )
      .attr("fill", tokyo95)
      .attr("stroke", "none")
    ...
    svgLine
      .append("text")
      .text("When will Equality in Labor Participation be achieved?")
      .attr("x", x(startText))
      .attr(
        "y",
        y(
    ...
    svgLine
      .append("text")
      .text(
        " Ratio of Labor Participation for Women to Labor Participation for Men"
      )
      .attr("x", x(startText))
      .attr(
        "y",
    ...
    svgLine
      .append("text")
      .text("Equity in Labor Participation")
      .attr("x", x(startText) + 10)
      .attr("y", y(1) - 15)
      .attr("text-anchor", "start")
      .attr("font-size", axisFontSize)
      .attr("font-family", "lato")
